...
class Picture:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 1:
            ...
            if isinstance(filename, str):
                ...
                raise TypeError(
                    "Picture: single input must be a filename, given: " + str(filename))
        elif len(args) == 2:
            wid = args[0]
            hgt = args[1]
            try:
                # will raise an exception if values are not numeric
                self._checkNumeric(wid, "__init__")
                ...
                raise TypeError(
                    "Picture: two inputs must be numeric, given: " + str(wid) + " and " + str(hgt))
        elif len(args) == 3:
            wid = args[0]
            hgt = args[1]
            color = args[2]
            try:
                ...
                raise TypeError(
                    "Picture: first two inputs must be numeric, given: " + str(wid) + " and " + str(hgt))
            try:
                self._checkColor(color, "__init__")
            except TypeError:
                raise TypeError(
                    "Picture: third input must be a valid color, given: " + str(color))
            self.width = wid
            self.hgt = hgt
            ...
            raise TypeError(
                "Picture: wrong number of arguments (must take 1, 2, or 3 inputs)")
    ...
    def save(self, filename):
        ...
        if isinstance(filename, str):
            ...
            raise TypeError(
                "(Picture) save: expected a string as input, giveN: " + str(filename))
    ...
    def show(self):
        ...
        if self.dispWindow is None:
            self.dispWindow = ImageWindow(
                self.image, self.width, self.height, self.filename)
        else:
    ...
    def drawArc(self, x0, y0, x1, y1, startAngle=0, endAngle=180, style='arc', outlineColor=(0, 0, 0), fillColor=None):
        ...
        if style not in {'arc', 'chord', 'pie'}:
            raise ValueError(
                "(Picture) drawArc: style must be one of 'arc', 'chord', or 'pie', given: " + str(style))
        # If all inputs okay, do drawing
        if self.drawObj is None:
            self.drawObj = ImageDraw.Draw(self.image)
        if style == 'arc':
            self.drawObj.arc([(x0, y0), (x1, y1)],
                             startAngle, endAngle, outlineColor)
        elif style == 'chord':
            self.drawObj.chord([(x0, y0), (x1, y1)], startAngle,
                               endAngle, fillColor, outlineColor)
        elif style == 'pie':
            self.drawObj.pieslice(
                [(x0, y0), (x1, y1)], startAngle, endAngle, fillColor, outlineColor)
    ...
    def _checkRange(self, x, y, funcName):
        ...
        if (0 <= x < self.width) and (0 <= y < self.height):
            ...
            if x < 0:
                ...
                errStr = "input x value out of bounds: {:d} >= {:d}".format(
                    x, self.width)
            elif y < 0:
                errStr = "input y value out of bounds: {:d} < 0".format(y)
            elif y >= self.height:
                errStr = "input y value out of bounds: {:d} >= {:d}".format(
                    y, self.height)
            else:
    ...
    def _checkNumeric(self, value, funcName):
        ...
        if isinstance(value, int) or isinstance(value, float):
            ...
            raise TypeError("(Picture) " + funcName +
                            ": expected value to be a number, given: " + str(value))
    ...
    def _checkColor(self, value, funcName):
        ...
        if isinstance(value, str):
            ...
            raise TypeError(errHeader + "Color must be tuple or string, not " +
                            str(value) + " of type " + str(type(value)))
...
def pickAFont():
    ...
    filename = tkFileDialog.askopenfilename(
        title="Select font file", filetypes=[('truetype font', '*.ttf')])
    if filename is None:
