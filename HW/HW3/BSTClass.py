    ...
    def _deleteRec(self, value):
        ...
        if value == self.nodeValue:
            if self.isLeaf():
                ...
                if result != None:
                    # update this node's left child
                    self.setLeftChild(result)
                return None                                # this node stays, so return None
        elif value < self.nodeValue:
            if self.hasLeftChild():
                # update this node's left child
                result = self.leftChild._deleteRec(value)
                if result != None:
                    self.setLeftChild(result)
            return None                                # this node stays, so return None
        else:  # value >= getNodeValue(tree)
            if self.hasRightChild():
                result = self.rightChild._deleteRec(
                    value)  # update this node's right child
                if result != None:
...
if __name__ == '__main__':
    ...
    tree.printTree()
    t2.printTree()
    print("tree isleaf?", tree.isLeaf(), "t2 is leaf?",
          t2.isLeaf(), "height", tree.getHeight())
    tree.inorderTraverse()
